...
//optional second param c, if true counts intermediate nodes else only counts leaf nodes.
...
jado.getkv = function(d,f,p) {
    ...
    f = _to(f) == "function" ? f : function(x){console.log(x)}; //hack for now
    /*return*/_rx(d,
        function(k,v,i){
            if (_il(v)) 
...
/* ==========================================
 ...
 for example
 jado.pathval({"this":"that","test":[1,2,3,4,[4,5]]},"this")     -->"that"
 jado.pathval({"this":"that","test":[1,2,3,4,[4,5]]},"test")     -->[1,2,3,4,[4,5]]
 jado.pathval({"this":"that","test":[1,2,3,4,[4,5]]},"test/3")   -->4
 jado.pathval({"this":"that","test":[1,2,3,4,[4,5]]},"test/5/1") -->5
 
 Note: if d is an atomic and path is "" its value is returned.
 eg
 jado.pathval(34,"")     --> 34;
 jado.pathval("this","") --> "this"
 */
jado.pathval = function(d,p) {
    ...
    try {
        ...
        if (_dbg)
            console.log("jado.pathval: bad path");
    }
    return val;
}
/* map deep e.g. getkv(data_object, function(path,val)
  keys with a / in them will be escaped, regex style eg
  "abc/this" : value --> "abc\/this"  

jado.mapd = function(d,f) {
    
    //var p = (_to(p)=="undefined") ? "" : p+"/"; ////var p = _v(p,"")+'/';
    var p = _ch(path,{"array":path,"string":function(path){return JSON.parse(p)}},[]);
    console.log(p);
    ...
    return _rx(d,
        function(k,v,i){
            k = k.toString().replace(/\\/g,'\\\\').replace(/\//g,'\\/');
            if (_il(v)) 
                v = jado.mapd(v,f,p+k); // jado.getkv(v,f,p+k,r);            
            //console.log(p+k,':',v);
            f(p.toString()+k.toString()+'-->'+v);
            return v;
        }
...
//jado.len   = function (x,c) {var cc=(c==true)?1:0; var f=function(a,i){return _il(a) ? _r(a,f,i+cc) : i+1}; return f(x,0);} 
jado.amapd   = function(d,f)  {var r=[], _amd=function(a,fd){ if(_il(a)) {_amd(a,fd)} else r.push(fd(a))}; _amd(d,f); return r;}


//if path is given use p2v to get reference to internal starting point.
//path can be in [,,,,,] form or string form
jado.mapdx = function(d,f) {
   f = _to(f) == "function" ? f : function(p,v){console.log(p+':'+v)}; //hack for now
   var path = []; 
   var __mdx = function(d,f,p) {
       
       return _mx(d,
           function(k,v,p) {
               console.log("mdx:"+k,v,p);
               p.push(k.toString());
               if(_il(v)) {
    ...
    return __mdx(d,f,path);
}

/* pathNorm 
   jado normalize a path to an array of indeces for traversal
 */
jado.pathNorm = function(path) {
    var r = [];
    if (_to(path) == "string") {
        try { 
    ...
    if (_il(path) == false)
        path = [path.toString()];
    if (_to(path) == "array")
        r = _m(path,function(x){ return x.toString()});
    
    return r;
}
/* path2value
*/
jado.p2v = function(d,p) {
    var val = {};
    p = jado.pathNorm(p);
...
/* if allow directory 'path' style paths
jado.pathb2paths = function(path) {
    return path.map(function(x){return x.toString().replace('\\','\\\\').replace('\/','\\\/');}).join('/');
}
*/
//=============================================================================
//=============================================================================
