...
//JADO  Jado A Data Output  - Punny recursive name :) really just wanted something 
...
})(typeof exports === 'undefined'? this['jado.js']={}: exports);
*/
...
jado.map       = function (d,f)   {var k, r=_ct(d); if(_il(_v(d))) for (k in d) {r[k]=f(_v(d[k]));} else r=f(_v(d));  return r; }; //updated for atomics
...
jado.mapx      = function (d,f)   {var k, r=_ct(d); if(_il(_v(d))) for (k in d) {r[k]=f(k,_v(d[k]));} else r=f(_v(d)); return r; }; //updated for atomics

// filter, used internally, works on [] or {}
jado.filter    = function (d,f)   {var k, r=_ct(d); if(_il(_v(d))) for (k in d) {if (f(d[k]  ) == true) {if (_to(r)=="array") {r.push(d[k])} else {r[k]=d[k]}}} else if (f(d)==true) r=d[k]; return r;}; //updated for atomics

// filterx(),  allows iterator to work on both key and value
// requires requires f to take 2 params: for arrays f(index,value), for  dicts f(key,value)
jado.filterx   = function (d,f)   {var k, r=_ct(d); if(_il(_v(d))) for (k in d) {if (f(k,d[k]) == true) {if (_to(r)=="array") {r.push(d[k])} else {r[k]=d[k]}}} else if (f(d)==true) r=d[k]; return r;}; //updated for atomics

// reduce(), used internally, works on [] or {}
jado.reduce    = function (d,f,i) {var k, r=i;  if(_il(_v(d))) for (k in d) {r = f(d[k],r);} else {r = f(d,r);}  return r;}; //updated for atomics
...
jado.reducex   = function (d,f,i) {var k, r=i;  if(_il(_v(d))) for (k in d) {r = f(k,d[k],r);} else {r = f(d,r);} return r;}; //updated for atomics
...
//tables are so useful they have their own function
